...
if(isset($_POST['submit'])){
	...
	header("Location:add-city-hotels.php?message=1");
	} }else{
	$iPageId = $_REQUEST['id'];
	$oProduct->getCityHotels($iPageId);
	...
	$iPageInsertId	       =	$oGeneral->update_data($sTableName,'fld_id', $aData,$iPageId);
				header("Location:add-city-hotels.php?id={$iPageId}&message=2&type=edit");
								...
								<form  method="POST" class='form-horizontal form-striped' enctype="multipart/form-data">
									...
									<div class="controls">
										<select name="fld_city"  class="input-xlarge">
												...
												<option <?=($aSubCategory[0]['fld_city']==$aCity[$i]['fld_id'])?'selected':'';?> value="<?=$aCity[$i]['fld_id']?>"><?=$aCity[$i]['fld_name']?></option>
												<?php } ?>
												</select>
									</div>
								</div>
									...
									<div class="control-group">
										<label for="textfield" class="control-label">Hotels Address</label>
										<div class="controls">
											<input type="text" name="fld_address" value="<?=$aSubCategory[0]['fld_address']?>" id="textfield" placeholder="Price Starts" class="input-xlarge">
										</div>
									</div>
