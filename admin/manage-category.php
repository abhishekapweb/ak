...
<head>
    ...
    function removeList(id)
	{
		 var x= confirm("Are you sure delete?");
			...
			$.post( "delete-category.php?id="+id, function( data ) {
									...
									<tbody>
										 <?php for($i=0;$i < $iCategory;$i++){?>
													<tr id="row<?=$aCategory[$i]['fld_id']?>">
														<td><?=$aCategory[$i]['fld_category']?></td>
														...
														<td><a class="btn menu-icon  vd_bd-grey vd_success" href="add-category.php?id=<?=$aCategory[$i]['fld_id']?>&type=edit">Edit</a>&nbsp;<a class="btn menu-icon  vd_bd-grey vd_grey" onClick="removeList(<?=$aCategory[$i]['fld_id']?>);">Delete</a></td>
													</tr>
                                                    <?php } ?>
