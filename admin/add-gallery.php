...
if(isset($_POST['submit'])){
	...
	header("Location:add-gallery.php?message=1");
	} }else{
	$iPageId = $_REQUEST['id'];
	$oProduct->getGallery($iPageId);
	...
	$iPageInsertId	       =	$oGeneral->update_data($sTableName,'fld_id', $aData,$iPageId);
				header("Location:add-gallery.php?id={$iPageId}&message=2&type=edit");
			...
			<div class="container-fluid">
				...
				<div class="breadcrumbs">
					<ul>
						...
						<li>
							<a href="forms-basic.html">Gallery</a>
							<i class="icon-angle-right"></i>
						</li>
						...
						<div class="box box-bordered">
							...
							<div class="box-content nopadding">
								<form  method="POST" class='form-horizontal form-striped' enctype="multipart/form-data">
									...
									<div class="controls">
										<select name="fld_trip_id"  class="input-xlarge">
												<option value="">Select Trip</option>
												<?php for($i=0;$i < $iCity;$i++){?>
												<option <?=($aSubCategory[0]['fld_trip_id']==$aCity[$i]['fld_id'])?'selected':'';?> value="<?=$aCity[$i]['fld_id']?>"><?=$aCity[$i]['fld_trip']?></option>
												<?php } ?>
												</select>
									</div>
								</div>
									<div class="control-group">
										...
										<div class="controls">
											<input type="text" name="fld_date" value="<?=$aSubCategory[0]['fld_date']?>" id="textfield" placeholder="yyyy-mm-dd" class="input-xlarge">
										</div>
									</div>
